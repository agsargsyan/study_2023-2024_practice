...
В алгоритме Adaptive RED (ARED) функция сброса модифицируется посредством изменения по принципу AIMD, заключающейся в том, что увеличение некоторой величины производится путём сложения с некоторым параметром, у уменьшение — путём умножения на параметр.)[@Rared:2006]
...
![График длины очередей на линке(R0-R1) при разных RED](image/av_queues_classic_gentle.eps){#fig:009  width=70%}

![График длины очередей на линке(R0-R1) при разных RED](image/av_queues_3types.eps){#fig:007  width=70%}

![График длины очередей на линке(R0-R1) при разных RED](image/queues_3types.eps){#fig:008  width=70%}
...
Смоделировали сеть при разных TCP(Reno, Vegas и Newreno)(@fig:011, @fig:012, @fig:010).

![График длины очередей на линке(R0-R1) при разных TCP](image/av_queues_tcp.eps){#fig:011  width=70%}

![График размера TCP окна для всех источников(TCP типа TCP/Reno)](image/TCP_75-150_classic.eps){#fig:012 width=70%}
